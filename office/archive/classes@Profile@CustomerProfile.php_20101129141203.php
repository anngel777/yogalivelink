...
class Profile_CustomerProfile extends BaseClass
{
    ...
    public function Execute()
    {
        ...
        $this->wh_id = 1000000002;
        echo "wh_id ==> $this->wh_id <br />";
    ...
    public function GetAndFormatSessions()
    {
        #$OBJ_SESSIONS = new Profile_CustomerProfileSessions();
        #return $OBJ_SESSIONS->ListTable();
    
        /*
        $where = ($this->load_all_records) ? '' : " AND `wh_id`='{$this->wh_id}'";
        
        $records = $this->SQL->GetArrayAll(array(
            'table' => $this->table_sessions_checklist,
            'keys'  => "{$this->table_sessions_checklist}.*, {$this->table_sessions}.*, {$this->table_instructor_profile}.*",
            'where' => "{$this->table_sessions_checklist}.active=1 $where",
            'joins' => "LEFT JOIN {$this->table_sessions} ON {$this->table_sessions}.sessions_id = {$this->table_sessions_checklist}.sessions_id 
                        LEFT JOIN {$this->table_instructor_profile} ON {$this->table_instructor_profile}.wh_id = {$this->table_sessions}.instructor_id
            ",
        ));
        
        if ($records) {
            #$output = ArrayToStr($records);
            ...
            $output .= "<br />" . $this->SQL->Db_Last_Query;
        }
    ...
    public function GetCommonActions()
    {
        ...
        $link_2     = "/office/dev_richard/class_execute;class=Profile_CustomerProfileFreeCredits;classVars={$this->wh_id}";
        $script_2   = "top.parent.appformCreate('Window', '{$link_2}', 'apps'); return false;";
        
        $output = "
        <div class='btn_actions'><a href='#' onclick=\"{$script_1}\">CANCEL ACCOUNT</a></div>
        <br />
    ...
    public function LoadTabs()
    {
        ...
        $OBJ_SESSIONS                       = new Profile_CustomerProfileSessions();
        $OBJ_CONTACTS                       = new Profile_CustomerProfileContacts($this->wh_id);
        $OBJ_TOUCHPOINTS                    = new Profile_CustomerProfileTouchpoints($this->wh_id);
        
        
        $date = date('His');
        
        $tab_content_profile                = $OBJ_CONTACTS->EditRecordSpecial($this->wh_id);
        $tab_content_purchases              = 'tab_content_purchases<br />===> ' . $date;
        $tab_content_sessions               = $OBJ_SESSIONS->GetAllSessions($this->wh_id); //ListTableText();
        $tab_content_touchpoints            = $OBJ_TOUCHPOINTS->GetAllTouchpoints($this->wh_id);
