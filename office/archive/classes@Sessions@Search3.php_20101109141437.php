<?php
class Sessions_Search3 extends BaseClass
{
    ...
    public function OutputInstructorPictureScript()
    {
        if ($this->instructor_picture_list) {
            ...
            <script type='text/javascript'>
                SetInstructorPictures('{$this->instructor_picture_list}');
            </script>";
        }
        
        /*
        if ($this->instructor_active_picture) {
            echo "
            <script type='text/javascript'>
                SetInstructorActivePicture('{$this->instructor_active_picture}');
            </script>";
        }
        */
    }
	...
	public function GetCourses($WH_ID=0, $DATE='')
	{
        ...
        $where_whid = ($WH_ID) ? "`instructor_id`='$WH_ID' AND " : "";
        
        $this->instructor_active_picture = ($WH_ID) ? $WH_ID : '';
        ...
        $records = $this->SQL->GetArrayAll(array(
            ...
            'keys'  => '*',
            'where' => "$where_whid `date`='$date' AND`active`=1",
        ));
        if ($this->ShowQuery) echo "<br />LAST QUERY = " . $this->SQL->Db_Last_Query;
        ...
        $instructor_picture_list .= "{$record['instructor_id']}|";
            ...
            $blust = isset($temp_records[$date][$time]) ? count($temp_records[$date][$time]) : 0;
            $z = $blust+1;
            
            
            $temp_records[$date][$time][$z]['sessions_id']              = $record['sessions_id'];
            $temp_records[$date][$time][$z]['ci_cancel_before_time']    = $record['notes'];
            $temp_records[$date][$time][$z]['ci_time_start']            = $record['start_datetime'];
            $temp_records[$date][$time][$z]['ci_time_end']              = $record['end_datetime'];
            
            $temp_records[$date][$time][$z]['instructor_id']            = $record['instructor_id'];
            $temp_records[$date][$time][$z]['instructor_name']          = $record['end_datetime'];
        }
    ...
    public function CreateSchedule()
    {
        ...
        for ($i=0; $i<25; $i++) {
            ...
            if (isset($existing_records[$divID])) {
                ...
                $display_a      = "<b>{$count}</b> {$count_title} AVAILABLE<br /><a href='#' onclick=\"ShowCourseListing('{$divID}', '{$instructors_list}');\">[ click to view ]</a>"; //$existing_records[$divID]['data'];
    ...
    public function CreateScheduleForInstructor()
    {
        ...
        for ($i=0; $i<25; $i++) {
            ...
            if (isset($existing_records[$divID])) {
                $display_a      = "<div style='padding-top:10px; color:#bbb;'><a href='#' onclick=\"ShowCourseListing('{$divID}'); SetInstructorPictures('{$instructors_list}');\">[ BOOK SESSION ]</a></div>";
                $classes        = ($count == 1) ? 'zones zone_editing_data' : 'zones zone_existing_data';
                $status         = 'existing';
                $recordID       = ''; //$existing_records[$divID]['id'];
            } else {
        ...
        foreach ($records as $record) {
            ...
            <li data-id='picture_{$record['wh_id']}' id='picture_{$record['wh_id']}_li'>
                ...
                <div class='picture_wrapper'>
                    ...
                    <span style='display:none;'>{$record['wh_id']}</span>
                    </a>
                </div>
                </div>
            </li>
...
SCRIPT;
    ...
    public function AddStyle()
    {
        ...
        */
        ...
        #instructors_active_selected_holder {
