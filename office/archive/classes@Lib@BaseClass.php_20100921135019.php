...
class Lib_BaseClass
{
    ...
    public  $Add_Submit_Name;       //the name of the submit button when adding
    public  $Edit_Submit_Name;      //the name of the submit button when editing
    ...
    public function ListTableText()
    {
        ...
        $RESULT = qqn("
        <br />
        <a id=`{$link_group_name}1` class=`tablink` href=`#` onclick=`setTabSearchTable(1,'$tab_group_name'); return false;`>Search</a>
        <a id=`{$link_group_name}2` class=`tabselect`   href=`#` onclick=`setTabSearchTable(2,'$tab_group_name', '$eq', '$idx'); return false;`>Table</a>
    ...
    public function GetExportBlock($query_link)
    {
        return '
        <div class="EXPORT_SELECTION">
            Export<br />
            <a class="EXPORT_BUTTON"
              title="Comma Separated Values" href="' . $this->Export_Link . '?t=csv&amp;q='. $query_link . '"
              target="_blank">CSV</a>
            <a class="EXPORT_BUTTON"
              title="Excel XML Format" href="' . $this->Export_Link . '?t=xml&amp;q='. $query_link . '"
    ...
    public function OutputTable($search_array, $num_rows, $start_row, $row_count, $rows_only=false, $primary_sort_order='', $primary_sort_direction='')
    {
        ...
        $_SESSION[$var] = strTo($this->SQL->Db_Last_Query, 'LIMIT');

        $RESULT .= '
            <tr class="TABLE_TITLE">
                <td colspan="'. $colcount. '">
                ' . $this->GetExportBlock($query_link) . '
        ...
        foreach ($posted_data as $key=>$operator) {
            ...
            if ($field and ($operator != 'All')) {
                ...
                if ($field == 'active') {
                    ...
                    $field = strTo($this->GetFieldTitleFromAlias($field), ' AS');
