...
# ========================================================================================
# CLASS :: Used for customers and instructors to update their contact information
# ========================================================================================

class Profile_ContactInformation extends BaseClass
{
    ...
    public function ListRecordSpecialInactive($WH_ID, $RETURN_FALSE=false)
    {
        $this->Search_Inactive = true;
        return $this->ListRecordSpecial($WH_ID, $RETURN_FALSE);
    }
    
    public function ListRecordSpecial($WH_ID, $RETURN_FALSE=false)
    {
        $this->WH_ID = $WH_ID;

        $active = ($this->Search_Inactive) ? 0 : 1;
        
        $record = $this->SQL->GetRecord(array(
            'table' => $this->Table,
            'keys'  => "{$this->Table}.*, {$GLOBALS['TABLE_timezones']}.*, {$GLOBALS['TABLE_salutations']}.salutation AS contact_salutation",
            'where' => "`wh_id`=$this->WH_ID AND $this->Table.active=$active",
        ...
        if ($record) {
            ...
            $output = ($RETURN_FALSE) ? false : "<h1>UNABLE TO LOAD PROFILE</h1>";
        }
    ...
    public function SuccessfulEditRecord($flash, $id, $id_field)
    {
		...
		echoScript("
			...
			top.parent.reloadLeftColumn();
            top.parent.reloadCustomerSessionsNotRated();
    ...
    public function UpdateCustomerTimezone($WH_ID='')
    {
        ...
        if ($record) {
            ...
            $_SESSION['USER_LOGIN']['LOGIN_RECORD']['tz_display']   = $record['tz_display'];
