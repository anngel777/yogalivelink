...
upgraded to PHP5 2009-11-23 by MVP
...
class MsWord {
    ...
    // mvp moved constants to the class from global
    const One_Cent      = 28.35;  //1cm= 28.35pt
    const A4_WIDTH      = 21.0;
    const A4_HEIGHT     = 29.7;
    const A5_WIDTH      = 14.8;
    const A5_HEIGHT     = 21.0;
    const LETTER_WIDTH  = 21.59;
    const LETTER_HEIGHT = 27.94;
    const OFFICE_WIDTH  = 21.59;
    const OFFICE_HEIGHT = 35.56;

    public $appName = 'MsDocGenerator';
    public $appVersion = '0.4';
    ...
    public $formatBuffer;
    public $cssFile;
    public $lastSessionNumber;
    public $lastPageNumber;
    public $atualPageWidth;
    public $atualPageHeight;

    public $tableIsOpen;
    public $tableLastRow;
    public $tableBorderAlt;
    public $tablePaddingAltRight;
    public $tablePaddingAltLeft;
    public $tableBorderInsideH;
    public $tableBorderInsideV;
     ...
     */
    public function __construct($pageOrientation = 'PORTRAIT', $pageType = 'LETTER', $cssFile = '', $topMargin = 2.5, $rightMargin = 2.5, $bottomMargin = 2.5, $leftMargin = 2.5)
    {
        $this->documentBuffer = '';
        $this->formatBuffer = '';
     ...
     */
    public function newSession($pageOrientation = NULL, $pageType = NULL, $topMargin = NULL, $rightMargin = NULL, $bottomMargin = NULL, $leftMargin = NULL){
        //not set now? then use document start values
        ...
        switch($pageOrientation){
            case 'PORTRAIT' :
                switch($pageType){
                    case 'A4' :
                        $this->atualPageWidth  = self::A4_WIDTH * self::One_Cent;
                        $this->atualPageHeight = self::A4_HEIGHT * self::One_Cent;
                        break;
                    case 'A5' :
                        $this->atualPageWidth  = self::A5_WIDTH * self::One_Cent;
                        $this->atualPageHeight = self::A5_HEIGHT * self::One_Cent;
                        break;
                    case 'LETTER' :
                        $this->atualPageWidth  = self::LETTER_WIDTH * self::One_Cent;
                        $this->atualPageHeight = self::LETTER_HEIGHT * self::One_Cent;
                        break;
                    case 'OFFICE' :
                        $this->atualPageWidth  = self::OFFICE_WIDTH * self::One_Cent;
                        $this->atualPageHeight = self::OFFICE_HEIGHT * self::One_Cent;
                        break;
                    default:
            ...
            case 'LANDSCAPE' :
                switch($pageType){
                    case 'A4' :
                        $this->atualPageWidth  = self::A4_HEIGHT * self::One_Cent;
                        $this->atualPageHeight = self::A4_WIDTH * self::One_Cent;
                        break;
                    case 'A5' :
                        $this->atualPageWidth  = self::A5_HEIGHT * self::One_Cent;
                        $this->atualPageHeight = self::A5_WIDTH * self::One_Cent;
                        break;
                    case 'LETTER' :
                        $this->atualPageWidth  = self::LETTER_HEIGHT * self::One_Cent;
                        $this->atualPageHeight = self::LETTER_WIDTH * self::One_Cent;
                        break;
                    case 'OFFICE' :
                        $this->atualPageWidth  = self::OFFICE_HEIGHT * self::One_Cent;
                        $this->atualPageHeight = self::OFFICE_WIDTH * self::One_Cent;
                        break;
                    default:
                        die("ERROR: PAGE TYPE ($pageType) IS NOT DEFINED");
                }
         ...
         //die("PAGE WIDTH,HEIGHT: [$pageOrientation][$pageType] $this->atualPageWidth, $this->atualPageHeight (" . self::LETTER_HEIGHT .')');
     ...
     */
    public function addParagraph($content, $inlineStyle = NULL, $className = 'normalText') {
        $style = '';
        if(is_array($inlineStyle)){
            foreach($inlineStyle as $key => $value)
                $style .= "$key: $value;";
        } elseif (!empty($inlineStyle)) {
        // MVP added to allow non-array styles
            $style = $inlineStyle;
        }
     ...
     */
    public function bufferImage($imagePath, $width, $height, $title = ''){
        $this->numImages++;
        $buffer = "<!--[if gte vml 1]>";
        if($this->numImages == 1){
        ...
        $buffer .= '<v:shape id="_x0000_i102' . $this->numImages . '" type="#_x0000_t75"
           style="width:' . $this->pixelsToPoints($width) . 'pt;
           height:' . $this->pixelsToPoints($height) . 'pt">
           <v:imagedata src="' . $imagePath . '" o:title="accessibilityIssues" />
           </v:shape><![endif]--><![if !vml]><img width="$width" height="' . $height . '" src="' . $imagePath . '"
            v:shapes="_x0000_i102' . $this->numImages . '"><![endif]>';
        return $buffer;
    }//end bufferImage()
     ...
     * @param $height: height to show image in pixels
     */
    public function addImage($imagePath, $width, $height, $title = ''){
     ...
     */
    public function startTable($inlineStyle = NULL, $className = 'normalTable'){
        $style = '';
        if(is_array($inlineStyle)){
            foreach($inlineStyle as $key => $value)
                $style .= "$key: $value;";
     ...
     */
    public function addTableRow($cells, $aligns = NULL, $vAligns = NULL, $inlineStyle = NULL, $classesName = NULL){
        if(! $this->tableIsOpen)
        ...
        $tableWidth = $this->atualPageWidth;// - ($this->leftMargin * self::One_Cent + $this->rightMargin * self::One_Cent);
