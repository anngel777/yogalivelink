...
MTL;
    ...
    function Keys($array, $assoc=true)
    {
        $RESULT = '';
        if (!empty($array)) {
            if ($assoc) {
    ...
    function OutputTable($search_array, $field_titles='', $table_setup='', $edit_title='', $edit_links='', $id='')
    {
    ...
    function IsUnique($table, $key='', $value='', $exclude='')
    {
    ...
    function GetNextValue($table, $key)
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetNextDate($table, $key, $format, $inc)
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetFieldValues($table, $key='', $where='', $case='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetFieldValuesLC($table, $key='', $where='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
        return $this->GetFieldValues($table, $key, $where, 'L', $joins);
    }

    function ViewRecord($table, $title_array='', $table_setup='', $keys='', $where='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function ViewRecordArray($record, $title_array, $table_setup='')
    {
        global $FieldValues;
    ...
    function GetRecord($table, $keys='', $where='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function UpdateRecord($table, $key_values='', $where='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetArrayAssoc($table, $reference_key='', $keys='*', $where='', $order='', $joins='', $limit='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetAssocArray($table, $key='', $value='', $where='', $key_case='', $joins='', $order='', $limit='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function Count($table, $where='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetFreq($table, $key='', $order=2, $order_direction='', $where='', $joins='', $limit='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetMax($table, $key='', $group='', $where='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function MaxValue($table, $key='', $where='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function FieldArray($table, $key='', $where='', $order='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function GetValue($table, $key='', $where='', $joins='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function IncValue($table, $key='', $where='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
    ...
    function AppendValue($table, $key='',  $value='', $where='', $delimiter='')
    {
        if (is_array($table)) extract($table, EXTR_OVERWRITE);
        ...
        if ($delimiter) {
            ...
            $query = "UPDATE $table SET $key=CONCAT($key, IF($key != '' AND $key IS NOT NULL, $delimiter, ''), $value) WHERE $where LIMIT 1";
        } else {
            $query = "UPDATE $table SET $key=CONCAT($key, $value) WHERE $where LIMIT 1";
        }
    ...
    function GetTables($function_name = 'GetTables')
    {
        if ($this->Database_Type == 'SQLITE') {
    ...
    function TableFieldTitleNames($table)
    {
        $RESULT = array();
        $tableinfo = $this->TableFieldInfo($table);
        if (count($tableinfo)>0) {
    ...
    function CopyRow($table, $exclude_list='', $change_list='', $where='')
    {
