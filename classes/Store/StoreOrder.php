...
class Store_StoreOrder
{
    ...
    public $Text = array(
        ...
        'CURRENCY_lOCATION_BUTTON_UPDATE' => 'Update',
...
<p style="font-weight:bold; font-size:1.1em; color:#080;">@EMAIL_SAVE_NOTICE@</p>
    ...
    // ------ XML for CC Processing GoEmerchant ------
    // ------ Merchant System -------
    public $Merchant_Id   = '';
    public $Merchant_Pin  = '';
    public $Merchant_Url  = '';
...
<TRANSACTION>
 <FIELDS>
  ...
  <FIELD KEY="recurring_type"></FIELD>
    ...
    // ------ Authorize.Net ------
    public $Authorize_Net_Login_Id  = ''; // if set will use Authorize.Net
    public $Authorize_Net_Trans_Key = '';

    public $Authorize_Net_Url      = 'https://secure.authorize.net/gateway/transact.dll';
    public $Authorize_Net_Test_Url = 'https://test.authorize.net/gateway/transact.dll';
    public $Authorize_Net_Testing = false;  //Authorize.Net testing flag


    public $Authorize_Net_Array = array(
        'version' => '3.1',
        'type' => 'AUTH_CAPTURE',
        'delim_date' => 'TRUE',
        'delim_char' => ',',
    ...
    public $Authorize_Net_Translate_Array = array(
        'CARD_NUMBER'           => 'card_num',
        'CARD_EXP'              => 'exp_date',
        'CARD_SECURITY_CODE'    => 'card_code',
        'CARD_ADDRESS1'         => 'address',
        'CARD_CITY'             => 'city',
        'CARD_STATE'            => 'state',
        'CARD_COUNTRY'          => 'country',
        'CARD_POSTAL_CODE'      => 'zip',
        'CARD_PHONE'            => 'phone',
    );
    ...
    public function  __construct()
    {
        $this->ClassInfo = array(
            ...
            'Created'     => '',
            'Version'     => '1.0',
            'Description' => 'Manage buying a product in the store - or session credit',
        );
    ...
    public function ProcessOrderPage()
    {
        ...
        if (Post('GETCC')) {
            ...
            $array = $this->ProcessBuyerForm();
            AddError($this->Error);
            // ----- put array into into session
            $_SESSION['STORE_ORDER']['BUYER_INFO'] = $array;
            $_SESSION['STORE_ORDER']['BUYER_INFO_TABLE'] = $this->Buyer_Table;
        }
    ...
    public function ProcessBuyerForm()
    {
        return ProcessForm(
    ...
    public function GetFinalTable()
    {
        ...
        $this->Final_Table = $table;  // should not have any errors if this far
    }
    ...
    public function SetCart($PN, $QTY=1) // function Sets a cart with a single part without needing to AddToCart
    {
        $this->Cart = array();
        $this->Cart[$PN] = $QTY;
    ...
    public function GetShoppingCart($shopping=true)
    {
        ...
        if (!empty($this->Cart)) {
            ...
            foreach ($this->Cart as $PN => $QTY) {
                ...
                $RESULT .= "<td$td_options>{$record['part_number']}</td>";
    ...
    public function GetLocationCurrencyForm($update_button = false)
    {
        ...
        if ($this->Show_Country_State_On_Cart or $this->Use_Currency_Conversion) {
            ...
            if ($update_button) {
                $form_data[] = 'submit|' . $this->Text['CURRENCY_lOCATION_BUTTON_UPDATE'] . '|UPDATE|class="submit_button"';
            }
    ...
    // ==================================== CREDIT CARD ====================================
    public function GoEmerchantCreditCardProcess()
    {
        // card processing
    ...
    {
        #if ($this->Authorize_Net_Login_Id) {
            return $this->AuthorizeNetCreditCardProcess();
        #} else {
        #    return $this->GoEmerchantCreditCardProcess();
        #}

    }

    public function AuthorizeNetCreditCardProcess()
    {
        $show_full_errors   = false;
        $show_authnet_info  = false;
        $testing_mode       = false;
        $authorize_only     = false;
        
        # special testing mode stuff
        if ($testing_mode) {
            $this->Authorize_Net_Login_Id  = '7a9n42PTrL';
            $this->Authorize_Net_Trans_Key = '92MP3g4yT2ZHvt7R';
        }
        
        # initialize the Authorize.net object
        $OBJ_SALE = new AuthorizeDotNet_AuthorizeNetAIM($this->Authorize_Net_Login_Id, $this->Authorize_Net_Trans_Key);
        $OBJ_SALE->setSandbox($testing_mode);
        
        # misc array stuff to prep the Authorize.net array
        $invoice_num = date("mdyHis") . '-' . rand(1, 99);
        
        $STORE_ORDER = Session('STORE_ORDER');
        $authorize_net_array = array(
            'first_name'        => strTo($this->Final_Array['CARD_NAME'], ' '),
            'last_name'         => strFrom($this->Final_Array['CARD_NAME'], ' '),
            'amount'            => ArrayValue($STORE_ORDER, 'BILL_TOTAL'),
            'email'             => $_SESSION['STORE_ORDER']['BUYER_INFO']['email_address'], //'richard@mailwh.com',
            'email_customer'    => true,
            'invoice_num'       => $invoice_num,
        );
        
        # map the form field names to authrize.net names
        foreach ($this->Authorize_Net_Translate_Array as $final_array_key => $authorize_net_key) {
            if (isset($this->Final_Array[$final_array_key])) {
                $authorize_net_array[$authorize_net_key] = $this->Final_Array[$final_array_key];
            }
        }
        
        # store the mapped names into the authorize.net object        
        $OBJ_SALE->setFields($authorize_net_array);
        
        # make call to Authorize.net
        if ($authorize_only) {
            $response = $OBJ_SALE->authorizeOnly();
        } else {
            $response = $OBJ_SALE->authorizeAndCapture();
        }
        ...
        if ($show_authnet_info) {
            $this->Error .= '<br />Authorize_Net_Login_Id ===> ' . $this->Authorize_Net_Login_Id;
            $this->Error .= '<br />Authorize_Net_Trans_Key ===> ' . $this->Authorize_Net_Trans_Key;
            $this->Error .= '<br />';
            $this->Error .= '<br />invoice_num ===> ' . $invoice_num;
            $this->Error .= '<br />';
        }
        
        if ($response->approved) {
            #$this->Error .= "<h1>Success! The test credit card has been charged!</h1>";
            #$this->Error .= "Transaction ID: " . $response->transaction_id;
            ...
            $this->Error .= 'YOUR TRANSACTION HAS BEEN DECLINED:';
            $this->Error .= '<br />Reason: ' . $response->response_reason_text;
            $this->Error .= '<br />Transaction ID: ' . $response->transaction_id;
            
            if ($show_full_errors) {
                $response->error_message = (isset($response->error_message)) ? $response->error_message : '*MSG*';
                $this->Error .= '<br />';
                $this->Error .= '<br />approved ===> ' . $response->approved;
                $this->Error .= '<br />response_code ===> ' . $response->response_code;
                $this->Error .= '<br />response_subcode ===> ' . $response->response_subcode;
                $this->Error .= '<br />response_reason_code ===> ' . $response->response_reason_code;
                $this->Error .= '<br />';
                $this->Error .= '<br />error_message ===> ' . $response->error_message;
            }
        }
    }
