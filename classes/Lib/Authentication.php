...
';
    ...
    public function SendPasswordEmail($password)
    {
        ...
        $subject = $this->Login_Title . ' - Account Password Reset';
    ...
    public function ResetPassword()
    {
        ...
        $logline    = "PASSWORD RESET|{$user}|{$pwd}|{$this->Message}{$this->Error}";
        $this->LogActionAttempt($logline);
    ...
    public function LogInProcess()
    {
        ...
        if ($this->CheckLogin($user, $pass)) {
            ...
            $logline = "LOGIN|$user|$pwd|Login Successful";
            $this->LogActionAttempt($logline);
        } else {
            // Log an error for the login
            $pwd = $this->ObfuscatePassword($pass);
            $logline = "LOGIN|{$user}|{$pwd}|{$this->Error}";
            $this->LogActionAttempt($logline);
        }
    }
    ...
    public function LogActionAttempt($LOG)
    {
