 ...
 <FIELDS>
  ...
  <FIELD KEY="recurring_type"></FIELD>
    ...
    public $Authorize_Net_Array = array(
        ...
        'x_delim_char' => ',',
        'x_encap_char' => "'",
        'x_relay_reponse' => 'FALSE',
    );

    public $Authorize_Net_Translate_Array = array(
        'CARD_NUMBER' => 'x_card_num',
        'CARD_EXP' => 'x_exp_date',
        'CARD_SECURITY_CODE' => 'x_card_code',
        'CARD_ADDRESS1' => 'x_address',
        'CARD_CITY' => 'x_city',
        'CARD_STATE' => 'x_state',
        'CARD_COUNTRY' => 'x_country',
        'CARD_POSTAL_CODE' => 'x_zip',
        'CARD_PHONE' => 'x_phone',
    );
    ...
    {
        $this->Authorize_Net_Array['x_login']           = $this->Authorize_Net_Login_Id;
        $this->Authorize_Net_Array['x_tran_key']        = $this->Authorize_Net_Trans_Key;
        $this->Authorize_Net_Array['x_invoice_num']     = $this->Order_Id;
        $this->Authorize_Net_Array['x_test_request']    = true;

        $STORE_ORDER = Session('STORE_ORDER');
        $authorize_net_array = array(
            'x_first_name' => strTo($this->Final_Array['CARD_NAME'], ' '),
            'x_last_name'  => strFrom($this->Final_Array['CARD_NAME'], ' '),
            'x_amount'     => ArrayValue($STORE_ORDER, 'BILL_TOTAL')
        );
        ...
        $url                = ($this->Authorize_Net_Testing)? $this->Authorize_Net_Test_Url : $this->Authorize_Net_Url;
        $query              = http_build_query($authorize_net_array);
        $process_result     = file_get_contents("$url?$query");

        $this->Error = '';
        $response = explode(',', $process_result);
        if ($response[0] == 1) {
            $this->Error .= 'query ===> ' . $query;  // note that this is not translated
            //return true;
        } else {
            $this->Error .= 'url ===> ' . $url;  // note that this is not translated
            $this->Error .= 'query ===> ' . $query;  // note that this is not translated
            $this->Error .= ArrayToStr($response);  // note that this is not translated
            #$this->Error .= $response[3];  // note that this is not translated
        }
    }
