...
class Profile_CustomerProfile extends BaseClass
{
    ...
    public $Tabs_Div_Prefix             = 'customer_profile_';
    public $Tabs_Function_Prefix        = 'CustomerProfile';
    ...
    public function  __construct()
    {
        $this->ClassInfo = array(
            ...
            'Created'     => '',
            'Version'     => '1.0',
            'Description' => 'Admin overview of a customer in back-office - tabbed window',
        );
        ...
        $this->OBJ_SESSIONS         = new Profile_CustomerProfileSessions();
        $this->OBJ_TOUCHPOINTS      = new Profile_CustomerProfileTouchpoints();
        $this->OBJ_HELPCENTER       = new Profile_CustomerProfileHelpCenter();
        $this->OBJ_TABS             = new Tabs('tab', 'tab_edit');
        //$this->OBJ_OVERVIEW         = new Profile_CustomerProfileOverview();
        $this->OBJ_OVERVIEW         = new Profile_ProfileOverview();
        $this->OBJ_SESSIONSEARCH    = new Sessions_Search();
        
        $this->OBJ_OVERVIEW->WH_ID      = $this->WH_ID;
        ...
        */
		...
		switch ($action) {
            ...
            case 'customer_email_subscriptions':
				$QDATA = GetEncryptQuery('eq');
				$WH_ID = ArrayValue($QDATA, 'wh_id');
				echo $this->OBJ_OVERVIEW->GetCustomerEmailSubscriptions($WH_ID);			
			break;
            
            case 'instructor_public_profile':
				$QDATA = GetEncryptQuery('eq');
				$WH_ID = ArrayValue($QDATA, 'wh_id');
				echo $this->OBJ_OVERVIEW->GetInstructorPublicProfile($WH_ID);			
			break;
            
            case 'left_column':
				$QDATA = GetEncryptQuery('eq');
				$WH_ID = ArrayValue($QDATA, 'wh_id');
				echo $this->OBJ_OVERVIEW->GetLeftColumnContent($WH_ID);			
			break;
    ...
    public function LoadTabContent($action)  // MVP function ------- used for ajax
    {
        ...
        switch ($action) {
            ...
            case 'purchases':
                $this->OBJ_PURCHASES->page_location = $this->URL_Purchases_View_Orders;
                $RESULT = $this->OBJ_PURCHASES->Execute('all', 0);
            break;
            ...
            case 'helpcenter':
                ...
                $content_right = $this->OBJ_HELPCENTER->Execute();
                $content_left = $this->OBJ_HELPCENTER->ColumnLeft();
                
                $RESULT = "
                <div>
                <div style='float:left;'>$content_left</div>
                <div style='float:left;'>$content_right</div>
            ...
            case 'session_search':
                $RESULT = $this->OBJ_SESSIONSEARCH->Execute();
            break;
