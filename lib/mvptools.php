<?php
// ============PHP Routines - Michael V. Petrovich=================
// ============REVISIONS============
/*
...
$THIS_PAGE       = preg_replace('/(;|\?|:).+$/', '', $THIS_PAGE_QUERY);
$HTTPS           = Server('HTTPS');
...
function GetPost($name) {return Request($name);}
function Request($name) {return (isset($_REQUEST[$name]))? $_REQUEST[$name] : '';}
...
function CheckEmail($email, $use_quotes=false)
{
    if (!$use_quotes) {
...
//------------ SEND EMAIL --------------
function SendHTMLmail($FromName, $FromEmail, $Recipientlist, $Subject, $Message, $CC='', $BCC='')
{
    global $MailCR;
...
function WriteFileFtp($ftp_server, $ftp_username, $ftp_password, $ftp_file_path,  $content)
{
...
function WriteFileFtpC($ftp_conn, $ftp_file_path, $content, $mode=FTP_BINARY)
{
    $tmp = tmpfile();
    fwrite($tmp, $content);
    fseek($tmp, 0);
...
function strIn($string, $within, $offset=0)
{
...
function strFromLast($string, $from) {
    if (empty($from) or empty($string)) return $string;
...
function FormatAddress($str, $br = '<br />')
{
...
function AssocArrayToStr($array)
{
    // deprecated use ArrayToStr($array)
    return ArrayToStr($array);
}

function ArrayToStr($array, $html=true)
{
    $RESULT = '';
...
function GetDirectory($url, $includestr='', $excludestr='', $subdirectories=true)
{
...
function GetDirCount($url, $includestr='', $excludestr='', $subdirectories=true)
{
...
function DecryptString($string, $key, $compress=2, $encode='64')
{
    if (!$string) {
        return '';
    }
...
function EncryptQuery($str)
{
    global $ENCRYPT_QUERY_KEY;
    return EncryptString($str, $ENCRYPT_QUERY_KEY);
}

function GetEncryptQuery($encrypted_query_var, $get_variable_name=true)
{
...
function astr_replace($array, $string, $ignore_case=false)
{
    global $USE_MB;
...
function mb_str_replace($old, $new, $string)
{
    $offset  = 0;
...
function mb_str_ireplace($old, $new, $string)
{
...
function readln($filename='', $SetCSV=false)
{
 ...
 */
function SendPost($host, $path, $data, $port=80, $useragent=0)
{
...
function GetPassword($size=8, $password_chars='abcdefghjkmnpqrtuvwxyz2346789')
{
    $RESULT = '';
