...
$FORM_VAR = array(
    ...
    'title_template'     => '<br class="formtitlebreak" /><div class="formtitle">@@VAR@@:</div>' ."\n",    // -------- Template for the title
    'info_template'      => '<div class="forminfo">@@VAR@@</div>'."\n\n",    // -------- templete for the input field
    ...
    'has_too_few_characters' => 'has too few characters',
    'has_too_many_characters' => 'has too many characters',
...
function WriteErrorText($error)
{
    ...
    $RESULT = '';
    if(!empty($error)) {
        list($first, $last) = explode('@@ERROR@@', $FORM_VAR['error_template']);
...
{
    global $FORM_VAR;
    //$RESULT = intOnly($phone);
    $RESULT = preg_replace('/[\(\) ]/', $FORM_VAR['phone_delimiter'], $phone);
    ...
    if ($value) {
        ...
        if (count($min_max)>1) {
            if (strlen($value) < $min_max[0]) {
                $error .= "$field1 {$FORM_VAR['has_too_few_characters']}$brk";
                $RESULT = false;
            }
            $max = $min_max[1];
        } else {
        ...
        if (strlen($value) > $max) {
            $error .= "$field1 {$FORM_VAR['has_too_many_characters']}$brk";
            $RESULT = false;
        }
    }
    return $RESULT;
...
function ProcessForm($formdata,&$table, $tableoptions, $thoptions, $tdoptions,&$error, $TBL=true) {
    ...
    $FORM_MONTHS, $FORM_STATE_CODES, $FORM_CANADA_PROVINCES,
    $FORM_BLOCK_REFERRER,
    ...
    foreach($formdata as $item) {
        ...
        case 'hh5':
        case 'hh6':
        case 'fieldset': //"fieldset|title|options_fieldset|options_legend"
            if ($TBL) {
                //"hx|text|options"
                if ($kind == 'fieldset') {
                    $kind = 'h3';
        ...
        case 'ehidden':
          //"ehidden|VARNAME|value|title"
            $value = DecryptString(GetPostItem($field1), $ENCRYPT_QUERY_KEY);
            if (!empty($field3)) {
                $tvalue = nl2br($value);
        ...
        case 'autocomplete':
            ...
            $value = GetPostItem($field2);
            $valueShown = TransformContent(Post("AC_$FormPrefix$field2"), 'TS');
            Form_CheckMissing($field3, $value, '', $field1, $error);
            if (!empty($field10) and $value) if (!preg_match("/$field10/", $value)) $error .= "$field1 value {$FORM_VAR['illegal_characters']}$brk";
            if (Form_OkRow($value) and $TBL) $table .= "<tr><th $thoptions>$field1</th>$n<td $tdoptions>$valueShown</td></tr>$n";
            $RESULT[$field2] = $value;
            break;
        ...
        case 'selecttext':
            ...
            $value = GetPostItem($field2);
            if (!Form_CheckListValue($value, $field, 7)) $value = '';             
            ...
            if ($value==$FORM_VAR['start_select_value']) {
                $value = '';
            } elseif ($value==$FORM_VAR['new_select_text_value']) {
                $value = $value2;
                Form_CheckMinMax($value, $field1, 5, $error);
            }
            if (Form_OkRow($value) and $TBL) $table .= "<tr><th $thoptions>$field1</th>$n<td $tdoptions>$value</td></tr>$n";
            $RESULT[$field2] = $value;
            break;
...
function Form_GetTitleCode($title, $title_template) {
    if (empty($title_template)) {
        ...
        if (empty($title)) {
            return str_replace('@@VAR@@:', '', $title_template);
        } else {
            list($first, $last) = explode('@@VAR@@', $title_template);
...
function Form_GetInfoCode($info, $info_template) {
    if (empty($info_template)) {
        ...
        list($first, $last) = explode('@@VAR@@', $info_template);
...
<test>
    ...
    <testvalue>25.00</testvalue><testvar>eHidden2</testvar>
        ...
        case 'ehiddeninfo':
            ...
            $RESULT .= "<input type=\"hidden\" id=\"$FormPrefix$field1\" name=\"$FormPrefix$field1\" value=\"$field2\" />$n$n";
            break;
...
<test>
    ...
    <testvalue>This is a test of long text to see what will happen</testvalue><testvar>test1</testvar>
        ...
        case 'qtext':
            ...
            $formitem = "<input type=\"text\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class$class2\" name=\"$FormPrefix$field2\" size=\"$field4\"$maxlen value=\"$value\"$options />$field9$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'integerc':
            ...
            $formitem = "<input type=\"text\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"$field4\"$maxlen value=\"$value\"$options />$field8$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
/*<formitem>
...
function formatRealObj(obj) {
    ...
    obj.value = base + rem;
            ...
            $class =  ($error)? 'formitemerror' : 'formitem';
            $formitem = "$&nbsp;<input type=\"text\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"$field4\" maxlength=\"$field5\" value=\"$value\"$options />$field7$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'autocomplete':
            ...
            $class =  ($error)? 'formitemerror' : 'formitem';
            $formitem  = "<input type=\"text\" alt=\"$alt\" id=\"AC_$FormPrefix$field2\" class=\"$class ac_input\" name=\"AC_$FormPrefix$field2\" size=\"$field4\" maxlength=\"$field5\" value=\"$value2\"$options />$field11$n";
            $formitem .= "$n<input type=\"hidden\" id=\"$FormPrefix$field2\" name=\"$FormPrefix$field2\" value=\"$value\" />$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            $complete_function = ($field9)? $field9 : "''";
            $JavaScript .= "$field7('AC_$FormPrefix$field2','$FormPrefix$field2','$field8',$complete_function);$n";
        ...
        case 'creditcard':
            ...
            $formitem = "<input type=\"text\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"$FORM_CREDITCARD_LENGTH\" maxlength=\"$FORM_CREDITCARD_LENGTH\" value=\"$value\"$options />$field5$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
...
<test>
    <code>phone|phone1|VARNAME|N||aftertext</code>
    <testvalue>123-333-1234</testvalue><testvar>phone1</testvar>
        ...
        case 'phone':
            ...
            $formitem = "<input type=\"text\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"12\" maxlength=\"20\" value=\"$value\"$options />$field5$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'qtextarea':
            ...
            $formitem = "<textarea class=\"$class\" id=\"$FormPrefix$field2\" name=\"$FormPrefix$field2\" cols=\"$field4\" rows=\"$field5\"$options>$value</textarea>$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
/*<formitem>
...
<test>
    ...
    <testvalue><h1>"test &amp; test"</h1></testvalue><testvar>html</testvar>
        ...
        case 'html':
            ...
            $class =  (Form_RequiredCheck($field3, $process) and ($process) and (!$value))? 'formitemerror' : 'formitem';
            $formitem = "<textarea class=\"$class\" id=\"$FormPrefix$field2\" name=\"$FormPrefix$field2\" cols=\"$field4\" rows=\"$field5\"$options>$outtext</textarea>$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'datecc':
            ...
            $formitem =  "{$FORM_VAR['month']} <select id=\"$monthvar\" class=\"$class\" name=\"$FormPrefix$monthvar\">$n";
            ...
            $formitem .= "{$FORM_VAR['year']} <select id=\"$yearvar\" class=\"$class\" name=\"$FormPrefix$yearvar\"$options>$n";
            $formitem .= "<option value=\"00\">{$FORM_VAR['start_select']}</option>$n";
...
<test>
    ...
    <testvalue>2009-04</testvalue><testvar>dateym</testvar>
            ...
            if ($endyear == $startyear) {
                $formitem = "<input type=\"hidden\" id=\"$yearvar\" name=\"$FormPrefix$yearvar\" value=\"$startyear\" />$n";
                $formitem .= $startyear;
            } else {
            ...
            $formitem .=  "&nbsp;{$FORM_VAR['month']}&nbsp;<select id=\"$monthvar\" class=\"$class\" name=\"$FormPrefix$monthvar\"$options>$n";
            $formitem .= $SELECT_START;
            for($i=1; $i<=12; $i++) {
                $selected = ($monthvalue == $i)? ' selected="selected"' : '';
                $optvalue = sprintf('%02d', $i);
            ...
            if ($kind == 'dateymd') {
                ...
                $formitem .=  "&nbsp;{$FORM_VAR['day']}&nbsp;<select id=\"$dayvar\" class=\"$class\" name=\"$FormPrefix$dayvar\"$options>$n";
                $formitem .= $SELECT_START;
                for($i=1; $i<=31; $i++) {
        ...
        case 'time':
            ...
            $formitem  =  "{$FORM_VAR['hour']} <select id=\"$hourvar\" class=\"$class\" name=\"$FormPrefix$hourvar\"$options>$n";
            $formitem .= $SELECT_START;
            for($i=0; $i<=23; $i++) {
                $optvalue = ($i<10)? "0$i" : $i;
            ...
            $formitem .=  " {$FORM_VAR['minute']}&nbsp;<select id=\"$minutevar\" class=\"$class\" name=\"$FormPrefix$minutevar\"$options>$n";
            $formitem .= $SELECT_START;
            for($i=0; $i<=59; $i++) {
                $optvalue = ($i<10)? "0$i" : $i;
...
<test>
    ...
    <testvalue>2009-05-28 18:01</testvalue><testvar>datetime</testvar>
        ...
        case 'datetime':
            ...
            $formitem .=
            "&nbsp;{$FORM_VAR['month']}&nbsp;<select id=\"$monthvar\" class=\"$class\" name=\"$FormPrefix$monthvar\"$options>$n";
            $formitem .= $SELECT_START;
            for($i=1; $i<=12; $i++) {
                $selected = ($monthvalue == $i)? ' selected="selected"' : '';
                $optvalue = sprintf('%02d', $i);
            ...
            $formitem .=
                "&nbsp;{$FORM_VAR['day']}&nbsp;<select id=\"$dayvar\" class=\"$class\" name=\"$FormPrefix$dayvar\"$options>$n";
            $formitem .= $SELECT_START;
            for($i=1; $i<=31; $i++) {
                $selected = ($dayvalue == $i)? ' selected="selected"' : '';
                $optvalue = ($i<10)? "0$i" : $i;
        ...
        case 'countrystate':
            ...
            function formCountryState(elem, stateVar) {
                var country_index = elem.selectedIndex;
                var country = '';
                if (country_index >= 0) {
                    country = elem.options[country_index].value;
                }
                var usState     = 'none';
                ...
                document.getElementById('{$FormPrefix}US_STATES_DIV_' + stateVar).style.display        = usState;
                document.getElementById('{$FormPrefix}CANADA_PROVINCES_DIV_' + stateVar).style.display = canadaState;
                document.getElementById('{$FormPrefix}OTHER_STATES_DIV_' + stateVar).style.display     = otherState;
            }";
            ...
            $JavaScript .= "formCountryState(document.getElementById('{$FormPrefix}$country_var'), '$state_var');";
            ...
            $formitem  =  "<select id=\"$FormPrefix$country_var\" class=\"$class\" name=\"$FormPrefix$country_var\" onchange=\"formCountryState(this, '$state_var')\">$n";
            $formitem .= $SELECT_START;
            Form_LoadCountryCodes();
            ...
            $RESULT .= '<div id="' . $FormPrefix . 'US_STATES_DIV_' . $state_var . '" style="display:block;">' . $n;
            ...
            $class =  (Form_RequiredCheck($field3, $process) and ($process) and ($value==$FORM_VAR['start_select_value']))? 'formitemerror' : 'formitem';
            $formitem  =  "\n<select id=\"{$FormPrefix}US_STATES_$state_var\" class=\"$class\" name=\"{$FormPrefix}US_STATES_$state_var\">$n";
            ...
            $RESULT .= '<div id="' . $FormPrefix . 'CANADA_PROVINCES_DIV_' . $state_var. '" style="display:none;">' . $n;
            ...
            $class =  (Form_RequiredCheck($field3, $process) and ($process) and ($value==$FORM_VAR['start_select_value']))? 'formitemerror' : 'formitem';
            $formitem  =  "\n<select id=\"{$FormPrefix}CANADA_PROVINCES_$state_var\" class=\"$class\" name=\"{$FormPrefix}CANADA_PROVINCES_$state_var\">$n";
            $formitem .= $SELECT_START;
            ...
            $RESULT .= '<div id="' . $FormPrefix . 'OTHER_STATES_DIV_' . $state_var. '" style="display:none;">' . $n;
            ...
            $error = ($process and (Form_RequiredCheck($field3, $process) and !$value));
            $class =  ($error)? 'formitemerror' : 'formitem';
            $formitem = "<input type=\"text\" alt=\"$state_var\" id=\"{$FormPrefix}OTHER_STATES_$state_var\" class=\"$class\" name=\"{$FormPrefix}OTHER_STATES_$state_var\" size=\"20\" maxlength=\"60\" value=\"$value\" />$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            $RESULT .= "</div>$n";
        ...
        case 'country':
            ...
            $RESULT .= "<div id=\"countrydiv_$field2\"$CountryDivOption>";
            $RESULT .= Form_GetTitleCode($field1, $TitleTemplate);
            $value = GetPostItem($field2);
            $ic = ($value)? $value : $FORM_VAR['default_country'];
            $class =  (Form_RequiredCheck($field3, $process) and ($process) and ($value==$FORM_VAR['start_select_value']))? 'formitemerror' : 'formitem';
            $formitem  =  "<select id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\"$options>$n";
            $formitem .= $SELECT_START;
            Form_LoadCountryCodes();
            foreach ($FORM_COUNTRY_CODES as $code => $name) {
                $selected =(($ic == $code) or ($ic == $name))? ' selected="selected"' : '';
        ...
        case 'state':
            ...
            $class =  (Form_RequiredCheck($field3, $process) and ($process) and ($value==$FORM_VAR['start_select_value']))? 'formitemerror' : 'formitem';
            $formitem  =  "\n<select id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\"$options>$n";
            foreach ($FORM_STATE_CODES as $code) {
                $statecode = (
    ...
    <testvalue>British Columbia</testvalue><testvar>intstate_int</testvar>
        ...
        case 'intstate':
         ...
         \"";
            ...
            $formitem  =  "<div class=\"withinformtext\" style=\"float:left; width:5.5em;\">{$FORM_VAR['us_state']}:</div><select id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\"$options$onchange>$n";
            ...
            $formitem .=  "<div id=\"{$field2}_span\"$IntStyle><span class=\"withinformtext\">or<br /></span>\n<div class=\"withinformtext\" style=\"float:left; width:5.5em;\">{$FORM_VAR['non_us']}:</div>\n<input type=\"text\" id=\"{$FormPrefix}INT_$field2\" class=\"$class\" name=\"{$FormPrefix}INT_$field2\" size=\"20\" maxlength=\"80\" value=\"$intvalue\"$options /></div>$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
    ...
    <testvalue>value2</testvalue><testvar>radioh</testvar>
        ...
        case 'radioh':
            ...
            for($i=5; $i<count($field); $i++) {
                ...
                if ($kind == 'radio') $BRK = ($i<count($field)-1)? '<br />' : '';
                else $BRK = ($i<count($field)-1)? '&nbsp;&nbsp;' : '';
                $idsuffix = '_'.($i-4);
        ...
        case 'select':
            ...
            for($i=$optionstart; $i<count($field); $i++) {
                ...
                if ($option !='') {
                    ...
                    $SelectItem = explode('=', $option);
                    $itemvalue = trim($SelectItem[0]);
                    $itemtext  = (count($SelectItem)>1)? trim($SelectItem[1]) : $itemvalue;
                    if (empty($itemtext)) {
                        $itemtext = $itemvalue;
                    }
    ...
    <testvalue>value3</testvalue><testvar>selecttext2</testvar>
        ...
        case 'selecttext':
            ...
            $options = "onchange=\"if (this.value == '{$FORM_VAR['new_select_text_value']}') {
              document.getElementById('new_$field2').style.display = '';
              } else document.getElementById('new_$field2').style.display = 'none';\"";
            ...
            $formitem .=  "<span id=\"new_$field2\" style=\"display:$style;\">&nbsp;{$FORM_VAR['new']} ($field1a): <input class=\"$class\" name=\"{$FormPrefix}new_$field2\" value=\"$value2\" type=\"text\" size=\"$field4\"$maxlen /></span>$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'checkboxlist':
          //"checkboxlist|title|options|value1=text|value2=text"
            ...
            for($i=3; $i<count($field); $i++) {
                ...
                $formitem .= "<input type=\"checkbox\" id=\"$FormPrefix$itemname\" class=\"formitem_checkbox\" name=\"$FormPrefix$itemname\" value=\"$itemvalue\"$checked$options /> $itemtext$BRK$n";
            }
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
...
<test>
    ...
    <testvalue>value2</testvalue><testvar>checkboxlistseth</testvar>
        ...
        case 'checkbox':
            ...
            $formitem .= "<input type=\"checkbox\" id=\"$FormPrefix$field2\" class=\"formitem_checkbox\" name=\"$FormPrefix$field2\" value=\"$field4\"$checked$options />$field6$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
        ...
        case 'password':
            ...
            $JavaScript .= "var My$field2 = '<input type=\"@@TYPE@@\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"$field4\" $maxlen value=\"@@VALUE@@\"$options />';$n";
           
            $formitem = "<span id=\"span_$field2\">
    <input type=\"password\" alt=\"$alt\" id=\"$FormPrefix$field2\" class=\"$class\" name=\"$FormPrefix$field2\" size=\"$field4\"$maxlen value=\"$value\"$options /></span>
        <br /><input type=\"checkbox\" value=\"1\" onclick=\"myElem  = document.getElementById('span_$field2');
        myValue = document.getElementById('$FormPrefix$field2').value;
...
<test>
    <code>submit|Submit|SUBMIT|onmousedown="this.style.backgroundColor='#f77'"</code>
        ...
        case 'submit':
            ...
            $formitem = "<input type=\"submit\" $class id=\"$field2\"  name=\"$field2\" value=\"$field1\" $field3 $onclick />$n";
            $RESULT .= Form_GetInfoCode($formitem, $InfoTemplate);
            break;
